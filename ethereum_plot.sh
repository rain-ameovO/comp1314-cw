...
db_name="crypto_db"
data_file="ethereum.txt"
graph_file="ethereum_price.png"
...
"SELECT DATE_FORMAT(price_time,'%Y-%m-%d_%H:%i:%s'), price FROM crypto_rate WHERE api_id='ethereum' ORDER BY price_time;" \
...
plot "$data_file" using 1:2 with lines linewidth 2 title "Ethereum Price"
EOF

echo "Ethereum Price graph has created"
